#!/usr/bin/env pwsh
# Avoid typed [switch] param for compatibility with -File invocation
$Json = $false
if ($args -and ($args -contains '-Json' -or $args -contains '-json')) { $Json = $true }
...
if ($Json) {
    ...
    # Manual JSON construction for CLM compatibility
